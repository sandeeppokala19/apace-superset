...
describe('Horizontal FilterBar', { testIsolation: false }, () => {
...
describe('Native filters', { testIsolation: false }, () => {
  ...
  describe('Nativefilters basic interactions', { testIsolation: false }, () => {
    before(() => {
      visitDashboard();
    });
  ...
  describe(
    'Nativefilters initial state not required',
    { testIsolation: false },
    () => {
      ...
      it('User can create a numerical range filter', () => {
        ...
        saveNativeFilterSettings([]);
        // assertions
        cy.get(nativeFilters.slider.slider)
          .should('be.visible')
          .click('center');
        cy.get(nativeFilters.applyFilter).click();
